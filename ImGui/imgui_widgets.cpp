...
bool ImGui::Button(const char* icon, const char* label, const ImVec2& size_arg)
{
    ...
    window->DrawList->AddText(bb.Min + ImVec2(49.f * g.IO.GUIScale / 2.f - CalcTextSize(icon).x / 2.f, bb.GetSize().y / 2.f - CalcTextSize(icon).y / 2.f), ImColor(Colors::Text), icon);
    PopFont();
    window->DrawList->AddText(bb.Min + ImVec2(49.f * g.IO.GUIScale, bb.GetSize().y / 2.f - label_size.y / 2.f), ImColor(Colors::Text), label);
...
bool ImGui::TabButton(const char* icon, const char* label, bool active) {
    ...
    ImVec2 size = ImVec2(180 * g.IO.GUIScale, 30 * g.IO.GUIScale);
...
bool ImGui::KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags, const char* text)
{
    ...
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(205 * io.GUIScale, 0), window->DC.CursorPos + ImVec2(308 * io.GUIScale, size.y));
    const ImRect total_bb(window->DC.CursorPos, frame_bb.Max);
    const ImVec2 text_bb(window->DC.CursorPos + ImVec2(20 * io.GUIScale, 0));
    ItemSize(size, style.FramePadding.y);
    ...
    RenderText(frame_bb.Min + ImVec2(7 * io.GUIScale, 0), text);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.0f + 5.f));
    ...
    TabItemMap->second.CheckboxFrame = ImLerp(TabItemMap->second.CheckboxFrame.Value, *v ? Colors::FrameActive : Colors::Frame, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed);
    TabItemMap->second.CircleFrame = ImLerp(TabItemMap->second.CircleFrame.Value, *v ? Colors::Accent : Colors::AccentDisabled, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed);
    TabItemMap->second.Text = ImLerp(TabItemMap->second.Text.Value, *v ? Colors::TextActive : Colors::Text, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed);
    TabItemMap->second.CirclePos = ImLerp(TabItemMap->second.CirclePos, *v ? 21.5f : 4.5f, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed);

    RenderNavHighlight(total_bb, id);
    RenderFrame(checkmark_bb.Min + ImVec2(0, 8.f), checkmark_bb.Max - ImVec2(0, 8.f), TabItemMap->second.CheckboxFrame, false, 12.f);
    window->DrawList->AddCircleFilled(checkmark_bb.Min + ImVec2(TabItemMap->second.CirclePos, checkmark_bb.GetSize().y / 2.f), 7.f, TabItemMap->second.CircleFrame);

    ImVec2 label_pos = ImVec2(total_bb.Min.x + 10.f, total_bb.GetCenter().y - CalcTextSize(label).y / 2.f);
    if (label_size.x > 0.0f)
        window->DrawList->AddText(label_pos, TabItemMap->second.Text, label);
...
bool ImGui::RadioButton(const char* label, bool active, int index)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
...
{
    const bool pressed = RadioButton(label, *v == v_button, 3);
    if (pressed)
        *v = v_button;
    return pressed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    TabItemMap->second.Border = ImLerp(TabItemMap->second.Border.Value, popup_open ? Colors::FrameOpened : Colors::Borders, g.IO.DeltaTime * 8.f * g.IO.AnimationSpeed);
    ...
    if (!(flags & ImGuiComboFlags_NoPreview)) {
        window->DrawList->AddRectFilled(combo_bb.Min, combo_bb.Max, TabItemMap->second.ComboFrame, 4.f, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll);
        window->DrawList->AddRect(combo_bb.Min, combo_bb.Max, TabItemMap->second.Border, 4.f, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    RenderFrame(ImVec2(slider_bb.Min.x, slider_bb.GetCenter().y - 1.5f), ImVec2(slider_bb.Max.x, slider_bb.GetCenter().y + 1.5f), ImColor(Colors::Borders), false, 8.f);
    ...
    // Render grab 
    {
        window->DrawList->AddRectFilled(ImVec2(slider_bb.Min.x, slider_bb.GetCenter().y - 1.5f), ImVec2(slider_bb.Min.x + (slider_bb.GetSize().x - 4.f) * AnimationHandle->second.position / 100.f, slider_bb.GetCenter().y + 1.5f), ImColor(Colors::Accent.x, Colors::Accent.y, Colors::Accent.z, Colors::Accent.w * 0.5f), 8.f);
        window->DrawList->AddCircleFilled(ImVec2(slider_bb.Min.x + (slider_bb.GetSize().x - 4.f) * AnimationHandle->second.position / 100.f, slider_bb.GetCenter().y), 7.f, ImColor(Colors::Accent), 24);
    }
    ...
    if (label_size.x > 0.0f)
        window->DrawList->AddText(ImVec2(frame_bb.Min.x + 10.f, frame_bb.GetCenter().y - CalcTextSize(label).y / 2.f), AnimationHandle->second.Text, label);
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_inputs)
    {
        if (RadioButton("RGB", (opts & ImGuiColorEditFlags_DisplayRGB) != 0, 3)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayRGB;
        if (RadioButton("HSV", (opts & ImGuiColorEditFlags_DisplayHSV) != 0, 3)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHSV;
        if (RadioButton("Hex", (opts & ImGuiColorEditFlags_DisplayHex) != 0, 3)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHex;
    }
    if (allow_opt_datatype)
    {
        if (allow_opt_inputs) Separator();
        if (RadioButton("0..255", (opts & ImGuiColorEditFlags_Uint8) != 0, 3)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Uint8;
        if (RadioButton("0.00..1.00", (opts & ImGuiColorEditFlags_Float) != 0, 3)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Float;
    }
