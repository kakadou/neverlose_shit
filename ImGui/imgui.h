...
namespace ImGui
{
    ...
    IMGUI_API bool          KeybindButton(const char* label, const char* unique_id, const ImVec2& size_arg, bool clicked, ImGuiButtonFlags flags, const char* text);
    IMGUI_API bool          ListBoxConfigArray(const char* label, int* currIndex, std::vector<std::string>& values, int height);
    ...
    IMGUI_API bool          RadioButton(const char* label, bool active, int index);                    // use with e.g. if (RadioButton("one", my_value==1)) { my_value = 1; }
