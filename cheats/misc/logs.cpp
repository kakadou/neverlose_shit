...
void eventlogs::events(IGameEvent* event) 
{
	...
	if (g_cfg.misc.events_to_log[EVENTLOG_HIT] && !strcmp(event->GetName(), crypt_str("player_hurt")))
	{
		...
		{
			ss << crypt_str("Harmed by ") << attacker_info.szName << crypt_str(" for ") <<event->GetInt(crypt_str("dmg_health")) << crypt_str(" in the ") << get_hitgroup_name(event->GetInt(crypt_str("hitgroup")));
...
#if RELEASE
#if BETA
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[neverlose.cc] ")); //-V807
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[neverlose.cc] "));
#endif
#else
		m_cvar()->ConsoleColorPrintf(g_cfg.misc.log_color, crypt_str("[neverlose.cc] ")); //-V807
#endif
...
#if RELEASE
#if BETA
		auto log = crypt_str("[ \x0Cneverlose.cc BETA \x01] ") + text;
		chat->chat_print(log.c_str());
#else
		auto log = crypt_str("[ \x0Cneverlose.cc \x01] ") + text;
		chat->chat_print(log.c_str());
#endif
#else
		auto log = crypt_str("[\x0CNEVERLOSE\x01] ") + text;
		chat->chat_print(log.c_str());
#endif
	}
}