...
void update_player_name()
{
	...
	static std::string username = m_cvar()->FindVar("name")->GetString();

	if (username != old_username)
	{
		old_username = username;
		g_ctx.username = username;
	}
}
...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (reload_fonts())
	{
		...
		fonts[LOGS] = create_font(crypt_str("Verdana"), 12, 700, FONTFLAG_DROPSHADOW);
	...
	if (panel_id == panel)
	{
		if (g_ctx.available())
		{
			...
			if (g_cfg.esp.molotov_wireframe)
			{
				static std::vector <std::string> molly_materials =
				{
					"particle/fire_burning_character/fire_env_fire.vmt",
					"particle/fire_burning_character/fire_env_fire_depthblend.vmt",
					"particle/fire_burning_character/fire_env_fire_depthblend_oriented.vmt",
					...
					"particle/fire_burning_character/fire_burning_character_depthblend.vmt",
					"particle/fire_burning_character/fire_molotov.vmt"
				...
				{
					auto material = m_materialsystem()->FindMaterial(material_name.c_str(), nullptr);

					if (!material)
						continue;

					bool fire = material_name.find("fire");

					material->SetMaterialVarFlag(MATERIAL_VAR_NO_DEBUG_OVERRIDE, !fire);
					material->SetMaterialVarFlag(MATERIAL_VAR_IGNOREZ, fire);
					material->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, fire);
				}
			}
