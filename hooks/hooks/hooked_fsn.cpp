			...
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					if (current_shot->impact_hit_player)
					{
						...
						//lagcompensation::get().player_resolver[current_shot->last_target].last_side = current_shot->side;

						if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
							eventlogs::get().add(crypt_str("missed shot due to animation desync [side: " + std::to_string(current_shot->side) + "]" + " [type: " + std::to_string(1) + "]"));

						if (lagcompensation::get().player_history[current_shot->last_target].history_available)
						{
							lagcompensation::get().player_history[current_shot->last_target].missed_shot_in_history_side = true;
							lagcompensation::get().player_history[current_shot->last_target].should_bruteforce = true;

							lagcompensation::get().player_history[current_shot->last_target].last_resolved_side = RESOLVER_ORIGINAL;
							lagcompensation::get().player_history[current_shot->last_target].last_resolved_angle = 0.0f;
						}
						else
						{
							lagcompensation::get().player_history[current_shot->last_target].missed_shot_in_history_side = false;
							lagcompensation::get().player_history[current_shot->last_target].should_bruteforce = true;
						}

						lagcompensation::get().player_history[current_shot->last_target].history_available = false;
						lagcompensation::get().player_history[current_shot->last_target].missed_shots += 1;
					}
				...
				else
				{
					...
					{
						lagcompensation::get().player_history[current_shot->last_target].missed_shot_in_history_side = false;
						lagcompensation::get().player_history[current_shot->last_target].should_bruteforce = false;

						lagcompensation::get().player_history[current_shot->last_target].last_resolved_side = (resolver_side)current_shot->side;
						lagcompensation::get().player_history[current_shot->last_target].last_resolved_angle = lagcompensation::get().player_resolver[current_shot->last_target].resolved_feet_yaw;
						lagcompensation::get().player_history[current_shot->last_target].history_available = true;
					}
				}
			}
